...
class Tank(GameObject):
    ...
    def draw(self, screen):
        ...
        # draw wheels
        for i in range(4):
            wheel_pos = np.array([tank_pos[0] - tank_width // 2 + (i + 1) * tank_width // 5, tank_pos[1] + tank_height // 2])
            pg.draw.rect(screen, self.color, [wheel_pos[0] - wheel_width // 2, wheel_pos[1] - wheel_height // 2, wheel_width, wheel_height])
        # draw tank body
...
class Manager:
    ...
    def handle_events(self, events):
        ...
        keys = pg.key.get_pressed()
        if keys[pg.K_LEFT]:
            self.gun.move_left(1)
        elif keys[pg.K_RIGHT]:
            self.gun.move_right(1)
        for event in events:
