...
class Bomb(GameObject):
    '''
    The bomb class. Creates a bomb, controls its movement, and implements its rendering.
    '''
    def __init__(self, coord, vel, rad=20, color=None):
        self.coord = coord
        self.vel = vel
    ...
    def move(self, time=1, grav=0):
        '''
        Moves the bomb according to its velocity and time step.
        Changes the bomb's velocity due to gravitational force.

        Parameters:
        - time (float): The time step for the movement (default: 1).
        - grav (float): The gravitational force applied to the bomb (default: 0).

        Returns:
        None
        '''
        self.vel[1] += grav  # Apply gravitational force to the bomb's vertical velocity
        for i in range(2):
            self.coord[i] += time * self.vel[i]  # Update the bomb's position based on velocity and time
        if self.vel[0]**2 + self.vel[1]**2 < 2**2 and self.coord[1] > SCREEN_SIZE[1] - 2*self.rad:
            self.is_alive = False  # Bomb is considered dead if its velocity is low and it's close to the bottom of the screen
