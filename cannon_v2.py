...
class RectangleTarget(GameObject):
    ...
    def __init__(self, coord=None, color=None, width=60, height=30):
        ...
        '''
        if coord == None:
            coord = [randint(width, SCREEN_SIZE[0] - width),
                     randint(height, SCREEN_SIZE[1] - height)]
        self.coord = coord
        self.width = width
        self.height = height
...
class Manager:
    ...
    def new_mission(self):
        ...
        for i in range(self.n_targets):
            self.targets.append(MovingCircleTarget(rad=randint(max(1, 30 - 2*max(0, self.score_t.score())),
                                                               30 - max(0, self.score_t.score())))),
            self.targets.append(MovingEllipseTarget(rad=randint(max(1, 30 - 2*max(0, self.score_t.score())),
                                                                30 - max(0, self.score_t.score())))),
            self.targets.append(CircleTarget(rad=randint(max(1, 30 - 2*max(0, self.score_t.score())),
                                                         30 - max(0, self.score_t.score())))),
            self.targets.append(EllipseTarget(rad=randint(max(1, 30 - 2*max(0, self.score_t.score())),
                                                          30 - max(0, self.score_t.score())))),
            self.targets.append(RectangleTarget(coord=[randint(200, SCREEN_SIZE[0] - 200), randint(200, SCREEN_SIZE[1] - 200)],
                                                color=rand_color(),
                                                width=randint(max(1, 30 - 2 * max(0, self.score_t.score())),
                                                              30 - max(0, self.score_t.score())),
                                                height=randint(max(1, 30 - 2 * max(0, self.score_t.score())),
                                                               30 - max(0, self.score_t.score()))))
            self.targets.append(MovingRectangleTarget(coord=[randint(200, SCREEN_SIZE[0] - 200), randint(200, SCREEN_SIZE[1] - 200)],
                                                color=rand_color(),
                                                width=randint(max(1, 30 - 2 * max(0, self.score_t.score())),
                                                              30 - max(0, self.score_t.score())),
                                                height=randint(max(1, 30 - 2 * max(0, self.score_t.score())),
